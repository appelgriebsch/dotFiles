...
description: >-
  ...
  Context: The user asked to implement a RabbitMQ message consumer in a Spring
  Boot microservice.

  user: "Write a RabbitMQ message listener for order events"

  assistant: "Here is the implementation of the RabbitMQ message listener:"

  <function call omitted for brevity>

  assistant: "Now let me use the spring-cloud-reviewer agent to review this
  implementation for correctness, error handling, and Spring AMQP best
  practices."

  <commentary>

  After writing the message listener, proactively launch the
  spring-cloud-reviewer agent to ensure the implementation follows Spring
  Messaging best practices, handles dead-letter queues, and uses proper
  ...
  Context: The user wants a review of a Spring Data JPA repository and service
  layer for a PostgreSQL-backed microservice.

  user: "Can you review this UserRepository and UserService I just wrote?"

  assistant: "I'll use the spring-cloud-reviewer agent to thoroughly review your
  UserRepository and UserService."

  <commentary>

  Since the user explicitly asked for a code review of Spring Data components,
  launch the spring-cloud-reviewer agent to analyze JPA query efficiency,
  transaction management, entity design, and data access patterns.
  ...
  Context: The user has implemented a new REST endpoint with Redis caching in a
  Kubernetes-deployed microservice.

  user: "I've added a new GET /products endpoint with Redis caching"

  assistant: "Let me use the spring-cloud-reviewer agent to review the endpoint
  implementation and caching strategy."

  <commentary>

  Proactively use the spring-cloud-reviewer agent to check cache key design, TTL
  configuration, cache eviction strategy, and Kubernetes-specific considerations
  like pod restarts affecting cache consistency.
  ...
  Context: The user has finished writing a PR that modifies a Spring Boot
  service's S3 integration.

  user: "I finished the PR for the S3 document upload feature, can you take a
  look?"

  assistant: "I'll launch the spring-cloud-reviewer agent to perform a thorough
  PR review of the S3 integration changes."

  <commentary>

  The user explicitly requested a PR review. Use the spring-cloud-reviewer agent
  to evaluate security (pre-signed URLs, bucket policies), error handling, and
  Spring Cloud AWS usage patterns.
