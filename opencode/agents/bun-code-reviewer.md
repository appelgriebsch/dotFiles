...
description: >-
  ...
  requests for Bun-based services and applications. Ideal for checking type
  safety, JSDoc documentation completeness, Biome linting compliance, correct
  usage of Bun built-in APIs (SQL/PostgreSQL, RabbitMQ, S3), security
  vulnerabilities, and adherence to idiomatic JS/TS patterns.


  <example>

  Context: The user has just written a new TypeScript repository module using
  Bun's built-in SQL driver to connect to PostgreSQL.

  user: "I've finished writing the user repository module in TypeScript using
  Bun's SQL driver."

  assistant: "Great, let me use the bun-code-reviewer agent to review your new
  module for type safety, SQL injection risks, idiomatic Bun patterns, and type
  annotation completeness."

  <commentary>

  Since the user just completed a new TypeScript module using Bun-specific SQL
  APIs, use the bun-code-reviewer agent to evaluate correctness, type safety,
  parameterized query usage, and documentation quality.
  ...
  Context: The user has a pull request adding S3 file upload support to an
  existing Bun JavaScript application.

  user: "Can you review my PR that adds S3 file upload support to our Bun app?"

  assistant: "I'll launch the bun-code-reviewer agent to review your S3 upload
  PR."

  <commentary>

  Since the user is requesting a PR review involving Bun's S3 integration in a
  JavaScript project, use the bun-code-reviewer agent to check error handling,
  JSDoc completeness, Bun S3 API correctness, and security considerations such
  as overly permissive ACLs.
  ...
  Context: A developer has just written a RabbitMQ consumer service in
  TypeScript using Bun and wants it checked before merging.

  user: "Here is my new RabbitMQ consumer service. Please review it before I
  push."

  assistant: "I'll use the bun-code-reviewer agent to review your RabbitMQ
  consumer implementation."

  <commentary>

  The user has completed a Bun/TypeScript service with RabbitMQ integration. Use
  the bun-code-reviewer agent to assess channel lifecycle management,
  acknowledgement patterns, type annotations, and Biome compliance.
