...
description: >-
  ...
  Context: The user asked for an Axum HTTP handler to be implemented and the
  assistant just wrote it.

  user: "Write me an Axum handler for user registration that stores data in
  PostgreSQL"

  assistant: "Here is the Axum registration handler with sqlx integration: [code
  implementation]"

  <commentary>

  Now that a non-trivial chunk of Rust code has been written, proactively launch
  the rust-code-reviewer agent to review it for correctness, idiomatic patterns,
  safety, and performance before delivering the final response.

  </commentary>

  assistant: "Now let me use the rust-code-reviewer agent to review this code
  ...
  Context: The user wants their existing Rust service code reviewed.

  user: "Can you review my Rust RabbitMQ consumer implementation?"

  assistant: "I'm going to use the rust-code-reviewer agent to thoroughly review
  your Rust message queue integration code."

  <commentary>

  The user is explicitly requesting a code review of Rust code involving
  RabbitMQ, so immediately trigger the rust-code-reviewer agent.
  ...
  Context: The user submitted a Rust PR diff for review.

  user: "Here's my PR diff for the S3 upload service, please review it"

  assistant: "I'll launch the rust-code-reviewer agent to perform a thorough
  review of this PR."

  <commentary>

  A PR review request for Rust code is the primary trigger for this agent. Use
  it immediately.
