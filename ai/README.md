...
| `brainstorm` | Analyze an improvement ticket or idea and produce a detailed implementation plan (challenges, tests, validation). If context is dubious/unclear or needs external facts, runs `research` first (writing/updating `RESEARCH.md` in the repo) before `grilling` the user on remaining open questions; always branches out to a mandatory `ask-the-expert` consult in Plan mode (whole-repo tech scan, all matched experts), then files every new GitHub issue with labels (`epic`, `has-plan`, `needs-brainstorm`, `needs-troubleshoot` as applicable). Posts a human-readable summary of the **full** plan and next steps (ASD-STE100 Simplified Technical English + ubiquitous language from `CONTEXT.md` when available) as a comment on the GitHub **main** issue and in the skill output. Plans only — never implements; run `implement-ticket` manually afterwards. |
...
| `implement-ticket` | Execute an implementation plan previously generated by the `brainstorm` or `troubleshoot` skills for a given GitHub Issue. **Hard-aborts** if `needs-brainstorm` or `needs-troubleshoot` remains on the ticket (or any EPIC sub-ticket); requires a body plan and prefers `has-plan`. For an EPIC (`epic` label and/or linked children), requires **every** sub-ticket ready (aborts and lists any that fail), creates a draft EPIC integration PR up front, sequences work by blocking edges, implements independent sub-tickets in parallel, and — once all sub-ticket PRs exist and are reviewed — stacks them onto the EPIC PR in dependency order via `gh-stack`. |
