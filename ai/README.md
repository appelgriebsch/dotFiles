...
| `brainstorm` | Plan an improvement ticket or idea into tracer-bullet work and tracker tickets. Ends with an STE summary on the main ticket, documented grilling decisions (ADRs or `RESEARCH.md`), readiness labels on every touched issue, and plan artifacts committed on the implement-ticket baseline branch with a draft PR. |
...
| `implement-ticket` | Execute a brainstorm/troubleshoot plan for a tracker ticket or EPIC (branch, implement, PR; one review on the final PR). Reuses the grooming baseline branch and draft PR when they exist, rebasing onto the current parent base first if that parent is not already an ancestor. For EPICs, require every sub-ticket plan, sequence by blockers, implement independent tickets in parallel, stack only the sub-ticket PRs onto the EPIC branch at the end using gh-stack (EPIC is the stack trunk, not a member targeting main), then review that stack once. |
| `issue-tracker` | Issue tracker identity, MCP operations, and git naming. Use when parsing a ticket id, fetching or creating a tracker issue, resolving parent/Epic or children, or naming a branch, commit, or pull request from a ticket. |
