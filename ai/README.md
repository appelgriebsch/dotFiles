...
| `brainstorm` | Analyze an improvement ticket or idea and produce a detailed implementation plan (challenges, tests, validation). If context is dubious/unclear or needs external facts, runs `research` first (writing/updating `RESEARCH.md` in the repo) before `grilling` the user on remaining open questions; always branches out to a mandatory `ask-the-expert` consult in Plan mode (whole-repo tech scan, all matched experts), then files every new GitHub issue. Posts a human-readable summary of the **full** plan and next steps (ASD-STE100 Simplified Technical English + ubiquitous language from `CONTEXT.md` when available) as a comment on the GitHub **main** issue and in the skill output. Plans only — never implements; run `implement-ticket` manually afterwards. |
| `convert-documents-to-markdown` | Convert Word, PowerPoint, Excel, OpenDocument, RTF, EPUB, CSV, and PDF files to GitHub-Flavored Markdown via the `anydoc` CLI (when office docs/spreadsheets/presentations/ebooks/PDFs cannot be read directly). |
...
| `implement-ticket` | Execute an implementation plan previously generated by the `brainstorm` or `troubleshoot` skills for a given GitHub Issue. For an EPIC with sub-tickets, requires **every** sub-ticket to have an implementation plan (aborts and lists any that lack one), creates a draft EPIC integration PR up front, sequences work by blocking edges, implements independent sub-tickets in parallel, and — once all sub-ticket PRs exist and are reviewed — stacks them onto the EPIC PR in dependency order via `gh-stack`. |
...
| `troubleshoot` | Analyze a problem or issue (via GitHub Issue or trace ID) and produce a detailed troubleshooting plan (causes, tests, validation). If context is dubious/unclear or needs external facts, runs `research` first (writing/updating `RESEARCH.md` in the repo) before `grilling` the user on remaining open questions; always branches out to a mandatory `ask-the-expert` consult in Diagnose mode (whole-repo tech scan, all matched experts; early Datadog evidence does not replace it), then files every new GitHub issue. Posts a human-readable summary of the **full** plan and next steps (ASD-STE100 Simplified Technical English + ubiquitous language from `CONTEXT.md` when available) as a comment on the GitHub **main** issue and in the skill output. Plans only — never implements; run `implement-ticket` manually afterwards. |
| `wait-what` | Stop and re-pitch the last message: add missing context, use ASD-STE100 Simplified Technical English, and prefer ubiquitous language from `CONTEXT.md` (not invoked automatically). |
...
flowchart TD
    ...
    G --> H["STOP — report plan + ticket URLs\n+ full-plan summary\n(posted to GitHub main issue)"]

    H -. "user runs manually" .-> I["implement-ticket"]
    I --> I0{"EPIC with sub-tickets?"}
    I0 -- "yes" --> I1{"Every sub-ticket\nhas a plan?"}
...
**No implicit handoff:** neither `brainstorm` nor `troubleshoot` may call `implement-ticket` or otherwise start building. The user must explicitly invoke `implement-ticket` for a filed ticket (or EPIC) to begin implementation. For EPICs, implementation starts only when **all** sub-tickets have plans.
