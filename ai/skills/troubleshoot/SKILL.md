...
**Always** invoke the `ask-the-expert` skill in **Diagnose** mode before finalizing — every run, not conditional on confidence, on the early Datadog pass, or on whether you already “know” the stack.
...
**Done when:** the plan has summary, reproduce steps (if applicable), ranked causes, fixes, and validation tests; [`research-grill-decisions.md`](../brainstorm/references/research-grill-decisions.md) **Done when** criteria are met; ticket/trace version signals were checked; `ask-the-expert` was actually invoked in Diagnose mode against the **resolved git tag/revision** (or an explicit current-workspace fallback); its Experts consulted / inventory outcome is reflected; feedback is incorporated.
